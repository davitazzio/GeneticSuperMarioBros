...
class FeedForwardNetwork(object):
    def __init__(self,
        ...
        self.hidden_activation = hidden_activation #relu
        self.output_activation = output_activation #sigmoid
        self.inputs = None
        self.out = None
        ...
        for l in range(1, len(self.layer_nodes)):
            if init_method == 'uniform':
                self.params['W' + str(l)] = np.random.uniform(-1, 1,
                                                              size=(self.layer_nodes[l], self.layer_nodes[l - 1]))
