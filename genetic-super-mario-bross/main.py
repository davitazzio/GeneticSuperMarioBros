...
class Visualizer(QtWidgets.QWidget):
    ...
    def _draw_region_of_interest(self, painter: QPainter) -> None:
        ...
        painter.drawRect(x * self.tile_width + 5 + self.x_offset, start_row * self.tile_height + 5,
                         viz_width * self.tile_width, viz_height * self.tile_height)
...
class MainWindow(QtWidgets.QMainWindow):
        ...
        if args.no_display:
            ...
            if args.load_inds:
                txt = "<font color='red'>" + str(
                    self.current_generation + 1) + '</font>'  # +1 because we switch from 0 to 1 index
    ...
    def next_generation(self) -> None:
        ...
        if not args.no_display:
            self.info_window.current_individual.setText(
                '{}/{}'.format(self._current_individual + 1, self._next_gen_size))
        ...
        if self.config.Selection.selection_type == 'plus':
            ...
            for individual in self.population.individuals:
                ...
                if lifespan > 0:
                    m = Mario(config, chromosome, hidden_layer_architecture, hidden_activation, output_activation,
                              lifespan)
                    # Set debug if needed
                    if args.debug:
        ...
        while len(next_pop) < self._next_gen_size:
            ...
            c1 = Mario(self.config, c1_params, p1.hidden_layer_architecture, p1.hidden_activation, p1.output_activation,
                       p1.lifespan)
            c2 = Mario(self.config, c2_params, p2.hidden_layer_architecture, p2.hidden_activation, p2.output_activation,
                       p2.lifespan)
    ...
    def _update(self) -> None:
        ...
        if self.mario.is_alive:
            ...
            if args.replay_file:
                ...
                if (
                        self.current_generation > self._true_zero_gen and self._current_individual == self._next_gen_size) or \
                        (
